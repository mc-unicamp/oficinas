...
class DCCCellImage extends DCCCell {
  ...
  createIndividualInitial (row, col, props) {
    ...
    if (light.properties != null) {
      if (light.properties.flip != null && light.properties.flip != 0) {
        light.element.setAttribute('transform',
          'scale(' + ((light.properties.flip == 1) ? '-1 1' : '1 -1') + ')')
      }
  ...
  updateElementState (element, properties, row, col) {
    ...
    if (properties != null) {
      if (properties.flip != null && properties.flip != 0) {
        light.element.setAttribute('transform',
          'scale(' + ((properties.flip == 1) ? '-1 1' : '1 -1') + ')')
      }
